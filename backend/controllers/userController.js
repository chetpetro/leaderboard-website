...
const loginUser = async (req, res) => {
    ...
    try{
        ...
        res.status(200).json({userName, discordID: user.discordID, token, isAdmin: user.isAdmin, newPoints: user.newPoints})
    } catch (err) {
...
const loginUserDiscord = async (req, res) => {
    ...
    try{
        ...
        res.status(200).json({userName: user.userName, discordID: user.discordID, token, isAdmin: user.isAdmin, newPoints: user.newPoints})
    } catch (err) {
        res.status(400).json({error: err.message})
    }
}
